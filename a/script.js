...
class NodeSelect extends HTMLElement
{
	constructor () {
		super();
	}
	...
	connectedCallback ()
	{
		...
		setTimeout( () =>
		{
			this.q("node-button").on("click", () =>
			{
				rootElement.toggleClass("node-select-active");
			});
		}, 20 );
	}
	...
	nodeSelect ( value )
	{
		...
		this.removeClass( "node-select-active" );
		this.q("node-button")[0].html( selectedOption.html() );

		// set class for selected option
		this.q("node-option").removeClass("selected");
		...
		this.displayFeed();
		this.dispatchEvent( new Event("node-select") );
	}
...
class NodeOptions extends HTMLElement
{
	constructor () {
		super();
	}
...
class NodeOption extends HTMLElement
{
	constructor () {
		super();
	}
	...
	{
		this.on("click", ()=>{
			this.closestParent("node-select").nodeSelect( this.attr("data-value") );
		})
		//super.connectedCallback();
	}
}
customElements.define("node-select", NodeSelect);
customElements.define("node-options", NodeOptions);
customElements.define("node-option", NodeOption);


q(()=>{

	let nodeSelects = q("node-select");
	document.body.on("click", (e)=>{
		for ( let nodeSelect of nodeSelects )
		{
			if ( e.composedPath().indexOf( nodeSelect ) === -1 ) {
				if ( nodeSelect.hasClass("node-select-active") ) nodeSelect.toggleClass("node-select-active");
			}
		}
	});
});
